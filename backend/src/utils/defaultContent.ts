// Ported from the frontend's lib/seed.ts + lib/helpers.ts so that a brand
// new database looks exactly like the old localStorage-seeded site did.

let counter = 1;
const uid = () => `id${counter++}_${Math.random().toString(36).slice(2, 7)}`;

function placeholder(label: string, bg = "#3f6b52", fg = "#ffffff") {
      ...
      sections: [
        {
          ...
          images: [placeholder("01", "#c9506b"), placeholder("02", "#4c7c59"), placeholder("03", "#caa53d"), placeholder("04", "#3f6b52")],
        },
        {
          id: uid(),
